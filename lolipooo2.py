			...
			if(state2 == None):
				...
				if(gameBoard.cellNb == len(gameBoard.myPlayer.myNodes)):
					...
					if(len(orderStack) > 0):
						...
						if(importantTarget.owner != gameBoard.myPlayer.id):
							...
							for node in nodeList:
								if(gameBoard.router.routes[node.id]['distance'] < maxDist):
									if(node not in nodeToExclude):
										if(node in gameBoard.myPlayer.frontNodes):
											neighboorsTypes = node.neighboorsType(gameBoard.myPlayer.id)
											toKeep = 0
											
											if((neighboorsTypes['ennemy'] + neighboorsTypes['neutral']) >= 1):
												if(neighboorsTypes['ennemy'] >= 1):
													tmp_target = None
													for neighboor in node.neighboors:
														if(neighboor.owner != -1 and neighboor.owner != gameBoard.myPlayer.id):
															tmp_target = neighboor
															break

													if(tmp_target.neighboorsType(gameBoard.myPlayer.id)['ally'] >= 2):
														for ally in tmp_target.neighboors:
															if(ally.owner == node.owner):
																orderStack.append((parse.createMoveOrder(ally,tmp_target.id,ally.unitAtq,playerId),19))
																nodeToExclude.append(ally)

													elif(neighboorsTypes['ennemy'] == 1):
														if(node.unitAtq > tmp_target.unitAtq + tmp_target.unitDef):
															orderStack.append((parse.createMoveOrder(node,tmp_target.id,node.unitAtq,playerId),19))
													
												else:
													tmp_target = None
													neededToCapture = -1
													for neighboor in node.neighboors:
														if(neighboor.owner == -1 and (neededToCapture == -1 or neighboor.unitAtq + neighboor.unitDef < neededToCapture)):
															tmp_target = neighboor
															neededToCapture = neighboor.unitAtq + neighboor.unitDef

													if(node.unitAtq > neededToCapture):
														orderStack.append((parse.createMoveOrder(node,tmp_target.id,neededToCapture + 1,playerId),5))
											...
											tmp_target = gameBoard.router.redirectionsForHelpers[node.id]
											if(tmp_target != None):
												place = tmp_target.maxAtq - tmp_target.unitAtq - 1
												if(place > 0):
									...
									if(len(gameBoard.myPlayer.myNodes) > nbNode):
										tmp_target = gameBoard.router.redirectionsForHelpers[node.id]
										neededToCapture = tmp_target.unitDef + tmp_target.unitAtq
										if(node.unitAtq > neededToCapture):
